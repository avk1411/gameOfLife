...
function drawGrid(size) {
    ctx.save();
    span = grid.width / size;
    //create one grid elem
    var canvasPattern = document.createElement("canvas");
    canvasPattern.width = span;
    canvasPattern.height = span;
    ...
    contextPattern.fillRect(0,0,span,span);
    contextPattern.strokeRect(0, 0, span, span);
...
function drawShape(shape) {
    ...
    shape.forEach(function (cell, i) {
        ctx.clearRect(cell[0]*span, cell[1]*span, span, span);
        ctx.fillRect(cell[0]*span, cell[1]*span, span, span);
        ctx.strokeRect(cell[0]*span, cell[1]*span, span, span);
    });
...
function clickCell(event) {
    ...
    var xVal = (event.pageX - grid.offsetLeft) / span;
    xVal = Math.floor(xVal);
    var yVal = (event.pageY - grid.offsetTop) / span;
    yVal = Math.floor(yVal);
    console.log(xVal, yVal);


    var imgData = ctx.getImageData((xVal * span) + (span/2) , (yVal * span) + (span/2), span, span).data;
    console.log(imgData);
    ...
    if(imgData[2]==0 || imgData[0]==255){
        ...
        ctx.clearRect(xVal * span, yVal * span, span, span);
        ctx.fillStyle = 'rgba(0, 0, 255, 1.0)';
        ctx.fillRect(xVal * span, yVal * span, span, span);
        ctx.lineWidth=1.0;
        ctx.strokeRect(xVal * span, yVal * span, span, span);
...
function countNeighbours(cell) {
    ...
    for(var i = -1;i<2;i++){
        for(var j = -1; j<2;j++){
            ...
            var xVal = cell[0] +i;
            var yVal = cell[1] +j;
            if(xVal > -1 && yVal> -1 && xVal <size && yVal<size){
                var imgData = ctx.getImageData((xVal * span) + (span / 2), (yVal * span) + (span / 2), span, span).data;
                if (imgData[2] != 0 && imgData[0] != 255) {
                    count++;
                }
